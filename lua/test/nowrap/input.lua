-- http://wiki.allegro.cc/index.php?title=Allegro_5_Tutorial/Input
...
al.al_register_event_source(event_queue, al.al_get_mouse_event_source())
...
while run do 
	...
	elseif (ev[0].type == al.ALLEGRO_EVENT_DISPLAY_CLOSE) then
		run = false
	elseif (ev[0].type == al.ALLEGRO_EVENT_MOUSE_AXES or ev[0].type == al.ALLEGRO_EVENT_MOUSE_ENTER_DISPLAY) then
		bouncer_x = ev[0].mouse.x 
		bouncer_y = ev[0].mouse.y
	elseif (ev[0].type == al.ALLEGRO_EVENT_MOUSE_BUTTON_UP) then
		run = false
	end 
